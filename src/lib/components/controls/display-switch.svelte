<script lang="ts">
    import { display } from '$lib/stores/controls';
    import { RadioGroup, RadioItem } from '@skeletonlabs/skeleton';

    let displayValue: 'Grid' | 'List' = 'Grid';
    $: {
        display.set(displayValue);
    }
</script>

<RadioGroup
    background="bg-surface-50 dark:bg-surface-800"
    border="border-none"
    class="hidden w-min text-slate-500 dark:text-slate-400 md:flex"
    color="variant-filled-primary"
    rounded="rounded-lg"
>
    <RadioItem bind:group={displayValue} label="Grid" name="sort-by" value={'Grid'}>
        <svg
            class={displayValue === 'Grid' ? 'h-4 text-white' : 'h-4'}
            viewBox="0 0 448 512"
            xmlns="http://www.w3.org/2000/svg"
        >
            <!--! Font Awesome Pro 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
            <path
                d="M0 80C0 53.49 21.49 32 48 32H144C170.5 32 192 53.49 192 80V176C192 202.5 170.5 224 144 224H48C21.49 224 0 202.5 0 176V80zM48 176H144V80H48V176zM0 336C0 309.5 21.49 288 48 288H144C170.5 288 192 309.5 192 336V432C192 458.5 170.5 480 144 480H48C21.49 480 0 458.5 0 432V336zM48 432H144V336H48V432zM400 32C426.5 32 448 53.49 448 80V176C448 202.5 426.5 224 400 224H304C277.5 224 256 202.5 256 176V80C256 53.49 277.5 32 304 32H400zM400 80H304V176H400V80zM256 336C256 309.5 277.5 288 304 288H400C426.5 288 448 309.5 448 336V432C448 458.5 426.5 480 400 480H304C277.5 480 256 458.5 256 432V336zM304 432H400V336H304V432z"
                fill="currentColor"
            />
        </svg>
    </RadioItem>
    <RadioItem
        bind:group={displayValue}
        class={displayValue === 'List' ? 'top-0 h-6 text-white' : 'top-0 h-6'}
        label="list"
        name="sort-by"
        value="List"
    >
        <svg class="h-4" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
            <!--! Font Awesome Pro 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. -->
            <path
                d="M88 48C101.3 48 112 58.75 112 72V120C112 133.3 101.3 144 88 144H40C26.75 144 16 133.3 16 120V72C16 58.75 26.75 48 40 48H88zM488 72C501.3 72 512 82.75 512 96C512 109.3 501.3 120 488 120H184C170.7 120 160 109.3 160 96C160 82.75 170.7 72 184 72H488zM488 232C501.3 232 512 242.7 512 256C512 269.3 501.3 280 488 280H184C170.7 280 160 269.3 160 256C160 242.7 170.7 232 184 232H488zM488 392C501.3 392 512 402.7 512 416C512 429.3 501.3 440 488 440H184C170.7 440 160 429.3 160 416C160 402.7 170.7 392 184 392H488zM16 232C16 218.7 26.75 208 40 208H88C101.3 208 112 218.7 112 232V280C112 293.3 101.3 304 88 304H40C26.75 304 16 293.3 16 280V232zM88 368C101.3 368 112 378.7 112 392V440C112 453.3 101.3 464 88 464H40C26.75 464 16 453.3 16 440V392C16 378.7 26.75 368 40 368H88z"
                fill="currentColor"
            />
        </svg>
    </RadioItem>
</RadioGroup>
