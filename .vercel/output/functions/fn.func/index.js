...
var init__ = __esm({
	'.svelte-kit/output/server/nodes/0.js'() {
		index = 0;
		component = async () =>
			(await Promise.resolve().then(() => (init_layout_svelte(), layout_svelte_exports))).default;
		file = '_app/immutable/entry/layout.svelte.b442a96c.mjs';
		imports = [
			'_app/immutable/entry/layout.svelte.b442a96c.mjs',
			'_app/immutable/chunks/index.f98f3a93.mjs'
		];
		stylesheets = [];
		fonts = [];
	}
...
var init__2 = __esm({
	'.svelte-kit/output/server/nodes/1.js'() {
		index2 = 1;
		component2 = async () =>
			(await Promise.resolve().then(() => (init_error_svelte(), error_svelte_exports))).default;
		file2 = '_app/immutable/entry/error.svelte.832893d9.mjs';
		imports2 = [
			'_app/immutable/entry/error.svelte.832893d9.mjs',
			'_app/immutable/chunks/index.f98f3a93.mjs',
			'_app/immutable/chunks/singletons.792809d8.mjs'
		];
		stylesheets2 = [];
		fonts2 = [];
	}
...
var init__3 = __esm({
	'.svelte-kit/output/server/nodes/2.js'() {
		index3 = 2;
		component3 = async () =>
			(await Promise.resolve().then(() => (init_page_svelte(), page_svelte_exports))).default;
		file3 = '_app/immutable/entry/_page.svelte.d93dad31.mjs';
		imports3 = [
			'_app/immutable/entry/_page.svelte.d93dad31.mjs',
			'_app/immutable/chunks/index.f98f3a93.mjs'
		];
		stylesheets3 = [];
		fonts3 = [];
			...
			switch (type) {
				...
				case 'Map':
					for (const [key2, value2] of thing) {
						keys.push(`.get(${is_primitive(key2) ? stringify_primitive$1(key2) : '...'})`);
						walk(value2);
						keys.pop();
					}
					break;
	...
	function stringify2(thing) {
		...
		switch (type) {
			...
			case 'Array':
				const members =
					/** @type {any[]} */
					thing.map((v, i) => (i in thing ? stringify2(v) : ''));
...
function serialize_data(fetched, filter, prerendering = false) {
	...
	for (const [key2, value] of fetched.response.headers) {
		...
		if (key2 === 'age') age = value;
		if (key2 === 'vary') vary = true;
	}
	const payload = {
...
`);
	...
	try {
		const strings = nodes.map((node) => {
			...
			return `{"type":"data","data":${stringify(node.data, reducers)},${stringify_uses(node)}${
				node.slash ? `,"slash":${JSON.stringify(node.slash)}` : ''
			}}`;
		});
		return {
...
function create_fetch({ event, options: options2, manifest: manifest2, state, get_cookie_header }) {
	return async (info, init2) => {
		...
		let credentials =
			(info instanceof Request ? info.credentials : init2?.credentials) ?? 'same-origin';
		return await options2.hooks.handleFetch({
			...
			fetch: async (info2, init3) => {
				...
				if (info2 !== original_request) {
					...
					credentials =
						(info2 instanceof Request ? info2.credentials : init3?.credentials) ?? 'same-origin';
				}
				if (
...
function validator(expected) {
	...
	function validate(module, file4) {
		...
		for (const key2 in module) {
			if (key2[0] === '_' || set.has(key2)) continue;
			const hint =
				hint_for_supported_files(key2, file4?.slice(file4.lastIndexOf('.'))) ??
...
var manifest = {
	...
	_: {
		client: {
			start: {
				file: '_app/immutable/entry/start.7574537f.mjs',
				imports: [
					'_app/immutable/entry/start.7574537f.mjs',
					'_app/immutable/chunks/index.f98f3a93.mjs',
					'_app/immutable/chunks/singletons.792809d8.mjs'
				],
				stylesheets: [],
			...
			app: {
				file: '_app/immutable/entry/app.b4cb91a9.mjs',
				imports: [
					'_app/immutable/entry/app.b4cb91a9.mjs',
					'_app/immutable/chunks/index.f98f3a93.mjs'
				],
				stylesheets: [],
				fonts: []
